...
void dynamic_benney_set(void) {
  fprintf(stderr, "ERROR: Benney dynamic not implemented in 3D\n");
  exit(1);
  // int i, j, k;
  //
  //
  ...
  //     for (k = 0; k < N; k++) {
  //       xk = ITOX(k);
  //       Psi[i][j] += actuator(xk-Aloc[j]) * (cos(K[i]*xk) -
  //       I*sin(K[i]*xk))/N;
  //     } // k end
  //
...
void dynamic_wr_set(void) {
  fprintf(stderr, "ERROR: WR dynamic not implemented in 3D\n");
  exit(1);
  ...
  // for (i = 0; i < M; i++) {
  //   J[i][M+i] = -(I*K[i]);
  //   J[M+i][i] = (5.0/RE) + (4.0/7.0 - 5.0/(3.0*RE*tan(THETA)))*(I*K[i]) +
  //   (5.0/(6.0*RE*CA))*(-I*K[i]*K[i]*K[i]); J[M+i][M+i] = -(5.0/(2.0*RE)) -
  //   (34.0/21.0)*(I*K[i]);
  // } // i end
  //
  //
  ...
  //     DYNAMIC_A[i][j] = J[i][j];
  //     for (k = 0; k < M; k++) {
  //       DYNAMIC_A[i][j] -= Psi[i][k] * DYNAMIC_K[k][j]; // NOTE opposite sign
  //       here
  //     } // k end
  //     for (k = 0; k < P; k++) {
  //       DYNAMIC_A[i][j] -= L[k][i] * Phi[j][k];
...
void dynamic_set(void) {
  fprintf(stderr, "ERROR: Dynamic not implemented in 3D\n");
  exit(1);
...
void dynamic_step(double dt, double *h, double *q) {
  ...
  //   Amag[i] = 0.0;
  //   for (int j = 0; j < M; j++) {
  //     Amag[i] += creal(DYNAMIC_K[i][j] * DYNAMIC_z[j]); // ensure this is
  //     real
  //   } // j end
  // } // i end
}
...
double dynamic_estimator(double x, double y) {
  fprintf(stderr, "ERROR: Dynamic not implemented in 3D\n");
  exit(1);
