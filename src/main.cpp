...
void get_exact_u(const double &t, const Vector<double> &x, Vector<double> &u) {
  ...
  complex<double> sol(exp(complex<double>(0.0, omega * t)) *
                      (exp(lambda * complex<double>(0.0, z)) -
                       exp(lambda * complex<double>(0.0, -z))) /
...
void get_exact_u(const double &t, const double &z, double &u) {
  // I=sqrt(-1)
  complex<double> I(0.0, 1.0);
  // omega
  double omega = 2.0 * MathematicalConstants::Pi;
...
public:
  ...
  /// lengths
  RayleighProblem(const unsigned &nx, const unsigned &ny, const unsigned &nz,
  ...
  void actions_before_implicit_timestep() {
    ...
    for (unsigned inod = 0; inod < num_nod; inod++) {
      ...
      double soln;
      ExactSoln::get_exact_u(time, z, soln);
...
RayleighProblem<ELEMENT, TIMESTEPPER>::RayleighProblem(
  ...
  bool periodic_in_y = true;
  Problem::mesh_pt() = new CubicBrickMesh<ELEMENT>(
      nx, ny, nz, lx, ly, lz, periodic_in_x, periodic_in_y, time_stepper_pt());
  ...
  for (unsigned ibound = 0; ibound < num_bound; ibound++) {
    ...
    for (unsigned inod = 0; inod < num_nod; inod++) {
      ...
      if ((ibound == 0) || (ibound == 5)) {
        mesh_pt()->boundary_node_pt(ibound, inod)->pin(0);
        mesh_pt()->boundary_node_pt(ibound, inod)->pin(1);
      }
      // Horizontal outflow on the left (and right -- right bc not
      // strictly necessary because of periodicity)
      else if ((ibound == 2) || (ibound == 4)) {
        mesh_pt()->boundary_node_pt(ibound, inod)->pin(1);
      }
      // Horizontal flow on the front (and back -- back bc not
      // strictly necessary because of periodicity)
...
void RayleighProblem<ELEMENT, TIMESTEPPER>::set_initial_condition() {
  ...
  if (time_stepper_pt()->type() != "BDF") {
    ...
    throw OomphLibError(error_stream.str(), OOMPH_CURRENT_FUNCTION,
  ...
  for (int t = nprev_steps; t >= 0; t--) {
    ...
    for (unsigned n = 0; n < num_nod; n++) {
      ...
      mesh_pt()->node_pt(n)->set_value(t, 2, soln[2]);
...
void RayleighProblem<ELEMENT, TIMESTEPPER>::doc_solution(DocInfo &doc_info) {
  ...
  sprintf(filename, "%s/error%i.dat", doc_info.directory().c_str(),
          doc_info.number());
  some_file.open(filename);
  mesh_pt()->compute_error(some_file, ExactSoln::get_exact_u, time_pt()->time(),
                           error, norm);
  some_file.close();
