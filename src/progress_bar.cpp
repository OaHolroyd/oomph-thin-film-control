...
ProgressBar::ProgressBar(int total, int width, void (*post_print)(void *),
                         float period, int rank) {
  this->total = total;
  this->progress = 0.0;
  this->period = period;
...
void ProgressBar::update(int progress, void *data, bool force) {
  ...
  // calculate progress
  this->progress =
      static_cast<float>(progress) / static_cast<float>(this->total);

  // compute elapsed time
  this->mid_event = std::clock();
  this->elapsed_time = 1000.0f * (float)(this->mid_event - this->start_event) /
                       (float)CLOCKS_PER_SEC;

  // decide whether to print
  if ((this->elapsed_time - this->last_update < this->period * 1000.0) &&
      !force) {
    return;
  }
...
void ProgressBar::update_bar() {
  ...
  // elapsed time
  fprintf(stderr, " elap: %s",
          time_format(this->elapsed_time / 1000.0f, SEC).c_str());
  ...
  fprintf(stderr, ", rem: %s",
          time_format(remaining_time / 1000.0f, SEC).c_str());
}